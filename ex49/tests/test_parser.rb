...
class ParserTests < Test::Unit::TestCase
  ...
  def test_paser_number()
    assert_equal(['number', 1], Parser.parse_number([['stop', 'at'], ['stop', 'the'], ['verb', 'walk']]))
    assert_equal(['number', 3], Parser.parse_number([['stop', 'at'], ['number', 3], ['verb', 'walk']]))
    assert_equal(['number', 3], Parser.parse_number([['number', 3], ['verb', 'walk']]))
    assert_equal(['number', 1], Parser.parse_number([['noun', 'bear'], ['verb', 'walk']]))
  end
